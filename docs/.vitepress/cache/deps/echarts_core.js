...
var Ring = function(_super) {
  ...
  Ring2.prototype.buildPath = function(ctx, shape) {
    ...
    ctx.arc(x, y, shape.r, 0, PI24, false);
    ctx.moveTo(x + shape.r0, y);
    ctx.arc(x, y, shape.r0, 0, PI24, true);
  };
  return Ring2;
}(Path_default);
...
function createScale(dataExtent, option) {
  ...
  var scale3 = createScaleByModel(axisModel);
  scale3.setExtent(dataExtent[0], dataExtent[1]);
  niceScaleExtent(scale3, axisModel);
  return scale3;
}
...
function projectPointToArc(cx, cy, r, startAngle, endAngle, anticlockwise, x, y, out2) {
  ...
  var ox = x * r + cx;
  var oy = y * r + cy;
  if (Math.abs(startAngle - endAngle) % PI23 < 1e-4) {
    out2[0] = ox;
    out2[1] = oy;
    return d - r;
  }
  ...
  if (angle >= startAngle && angle <= endAngle || angle + PI23 >= startAngle && angle + PI23 <= endAngle) {
    out2[0] = ox;
    out2[1] = oy;
    return d - r;
  }
